...
let rec insert x k seznam =
  match seznam with
  | [] -> x :: []
  | glava :: rep -> if (k <= 0) then x :: glava :: rep else glava :: insert x (k - 1) rep
...
let rec divide k list =
  ...
  | (k, glava :: rep) ->
    if k <= 0
      ...
      let (list1, list2) = divide (k - 1) (glava :: rep) in
        (glava :: list1, list2)
...
let rec rotate n list =
  ...
  | (0, _) -> list
  | (_, []) -> []
  | (_, glava :: rep) -> rotate (n - 1) (rep @ [glava])
...
let rec last_element list =
  ...
  | glava :: rep ->
    if rep = []
      then glava
    else last_element rep

let rec without_last list =
  match list with
  | [] -> []
  | glava :: rep -> if rep = [] then [] else glava :: without_last rep
...
let rec is_palindrome list =
  ...
  | glava :: rep ->
    if rep = []
      then true
    else (glava = last_element rep) && is_palindrome (without_last rep) 
...
let rec max_on_components list1 list2 =
  ...
  | (glava1 :: rep1, glava2 :: rep2) ->
    if glava1 < glava2
     then glava2 :: max_on_components rep1 rep2
    else glava1 :: max_on_components rep1 rep2
...
let rec max_value list =
  match list with
  | [] -> failwith "Prekratek seznam."
  | glava :: [] -> glava
  | glava :: rep ->
    if max_value rep <= glava
      then glava
    else max_value rep

let rec second_largest list = max_value (remove (max_value list) list)